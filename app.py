...
try:
    ...
    with st.sidebar:
        ...
        st.write("Обучена на 8,000+ делах")
    ...
    if submitted:
        ...
        if debtor_type == 'ФЛ':
            st.write("🔻 **Тип должника (ФЛ)**: значительно снижает вероятность (19.8% в среднем)")
        elif debtor_type == 'ИП':
            st.write("🔸 **Тип должника (ИП)**: умеренно снижает вероятность (42.5% в среднем)")
        else:
            st.write("🔺 **Тип должника (бизнес)**: повышает вероятность (60.3% в среднем)")
